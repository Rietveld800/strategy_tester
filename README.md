...
| `build_1m_rule1_report.py` | The rule 1 sweep (2026-08-26): the tested-reversal count at 2/3/4/5 on the `variant 2` and `variant 5` configurations, eight engine passes (~5-10 min), no cell cache. Output `quickfix1m1dcRule1.{json,html}`; the page carries its own update button (`reversals1m`, by key only, like the R-cut grids) and `--page` redraws without a backtest. The rule-1 = 3 rows are variants 2 and 5 re-measured, and the build prints a MATCH/DIFFERS check against the matrix JSON. |
