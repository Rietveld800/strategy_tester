...
| `research_1m_sizing.py` | The quantized-sizing reading (2026-09-01): the published blotter and the matrix's `variant 5` replayed in INTEGER contracts (`n = floor(equity x risk% / (rpu x point_value))`, force-1 at n=0) against the idealized fractional replay, at a test account of `--account` (default $2,000,000 -- Lode's choice, enough for one contract per market inside 1%). Specs come from data_center's validated `metadata/contract_specs.json`; no engine passes, seconds. First reading (window to 2026-08-31): at $2M quantization is noise (final within 0.25%, DD within 0.05 points, realized risk median 0.99%), one forced trade left on the baseline; the ETF capital lock is account-size INDEPENDENT (`locked/equity = risk% x price/rpu`) and 5 of 12 ETF trades lock over 100% of the account -- the quantitative case in the futures-only-portfolio decision. Writes `output/quickfix1m1dc_sizing.txt`. |
