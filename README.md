...
| File | What it does |
|---|---|
| `engine_1m.py` | The 1-minute engine: rules 1–2 intraday, market-order entry on the first reversal print, ladder-anchored stop, exit at the next settlement. `tests/test_engine_1m.py` covers it. `stop_live` (2026-09-03) runs the same entries with no stop order resting: the stop price still sizes the position and denominates R, the position is carried to the next settlement whatever the path did, and a loss is uncapped. |
...
| `build_1m_report.py` | `output/quickfix1m1dc_report_variant_01.html` (named for its cell) from the blotter alone (no backtest), and `--variant "<cell>"` for any matrix cell under its own filename. Imports `build_equity_html.CSS` so the look cannot drift. `--contracts` (2026-09-01) renders the same two trade lists (baseline + `variant 4`) with the money in INTEGER contracts at the $250k deployment account, as `quickfix1m1dc_contracts_variant_01.html` / `_04.html` -- sizing arithmetic imported from `research_1m_sizing` (one code path), specs from data_center's `contract_specs.json`, R columns untouched by construction, KPI row states the delta against the fractional ideal at the same account. SINCE 2026-09-03 THE MONEY IS THE MICRO STACK (Lode: the pages were still refusing orders as if only full contracts existed): `replay_micro` -- full contracts first, the routed micro topping the position up toward the 1% budget, drift priced, micro stop rounded away on its own grid -- lives here beside `replay_contracts` and is the SAME function the capital ladder's $250k rung runs, so the two pages agree to the cent. Full-contracts-only stays as the thin gray reference curve and a comparison tile; the blotter carries `Stack` (n full + k micro), the ACTUAL `Risk %` of equity the stack took at entry, and `Costs $` (both legs plus drift, split on hover); the refused table names the micro route. Without the gates JSON the page falls back to full-only and says so. The page opens with THE CONTRACT ARSENAL (same day): all 25 subscribed Socrates markets with front, size, tick, cost, routed micro (or why none) and a derived status -- Traded / In arsenal, no trade / Not backtested (failed the human filter, or no bars) / Not tradable (ETF, JGB) -- which is why the engine's 15 human-filter markets show against the account's 22; the "Not tested" list is dropped there. Refresh step `contracts1m` runs it in the full chain. SINCE 2026-09-03 THE CONTRACTS PAGES CARRY TWO ACCOUNTS (Lode: the curve without stop is as important as the curve with stop): the same account block -- KPIs, statistics, the three panes, exit classes, refused orders, open positions, blotter, per-market table, daily calendar -- rendered first for the NO-STOP run (the matrix's `nostop` companion of the cell: engine dial `stop_live` off, identical entries and sizing, every position carried to the next settlement) and then for the stopped run, same money layer, same universe, one shared time axis. The baseline page crosschecks the matrix cell against the published blotter first; no-stop rows link to the study through the stopped trade sharing their entry. `--contracts` also writes each cell's `_without_mf` twin (same evening, Lode): the human market filter lifted, from the matrix's `no-mf` / `no-mf no-stop` companions -- ZC, ZN, ZB, SB, FGBL and BTC join, SR3 has no bars, ETFs and JGB stay out; the arsenal says so. |
| `run_1m_matrix.py` | The dial matrix: the full lockout x stop x band cross on the filtered universe (`variant 1` .. `variant 27`; the extras were retired 2026-08-18; the band ladder is SHARED AND UNIFORM 000-060 / 020-060 / full since 2026-09-03, when the published cells were renumbered to variants 1 and 4). Since 2026-08-21 it is incremental per market (`output/quickfix1m1dc_matrix_cache.json`, `--no-reuse` to bypass): unchanged markets are reused whole, grown ones rerun only the tail with the overlap verified, and rows/account fields are derived canonically from the cached trades so no mode can drift. Curves **levered to a constant 6% max drawdown** with the risk solved per cell (the table carries both bases). Since 2026-09-03 the pass also runs THREE COMPANIONS of each report cell (`COMPANION_SPECS`: `no-stop` = the cell's dials with `stop_live=False`; `no-mf` = the dials on every eligible future, the human market filter lifted; `no-mf no-stop` = both; cached and spliced like cells, four extra engine passes per recomputed market) and writes them under `companions[cell][tag]` -- not cells: no number, no row, no charter slug. |
...
| `sharpe_stats.py` | Pure Sharpe statistics, no project imports: Lo (2002) standard error, PSR/DSR (Bailey & López de Prado), expected-max-SR deflation benchmark, participation-ratio and average-correlation effective N, day-cluster bootstrap. Pinned by `tests/test_sharpe_stats.py`. |
| `research_1m_sharpe.py` | The Sharpe **reading** of the R-cut grids (no engine passes, seconds): per-cell SR with error bars, the flag grid of what is distinguishable from what, the band-vs-no-band paired bootstrap, and the **Deflated Sharpe** with the effective-N estimate and break-even N*. Prints and writes `output/quickfix1m1dc_sharpe_<universe>.txt`; warns when a grid and the matrix were built on different data windows. First reading 2026-08-22: the grid is one blob, the band beats no band on both anchors (P ≤ 0.02), effective N ≈ 2 so the search could not have manufactured it, and the lower cut adds ~0.02 SR. |
| `build_1m_rule1_report.py` | The rule 1 sweep (2026-08-26): the tested-reversal count at 1-5 on the `variant 1` and `variant 4` configurations (both at band 000-060 since 2026-09-03), ten engine passes (~5-10 min), no cell cache. At 1 the setup arms on the first touch of the first reversal itself (the touch bar still needs its close back beyond the level). Output `quickfix1m1dcRule1.{json,html}` plus one page per month (`quickfix1m1dcRule1_<month>.html`, the sweep's trades partitioned by entry month - they sum exactly to the full window, no extra engine passes); the main page carries its own update button (`reversals1m`, by key only, like the R-cut grids), a per-market table per anchor (best count shaded, thin rows dimmed) and `--page` redraws everything without a backtest. The rule-1 = 3 rows are variants 1 and 4 re-measured, and the build prints a MATCH/DIFFERS check against the matrix JSON. |
| `research_1m_rule1.py` | The evidence reading of the rule 1 sweep (no engine passes, seconds): the paired day-cluster bootstrap of rule 3 against every other count (net R and per-trade SR, no selection deflation - rule 3 is the pre-existing rule, not a swept optimum), the marginal-trade decomposition by (market, session, side), and the rolling r3-minus-rN difference. Prints and writes `output/quickfix1m1dcRule1_research.txt`. First reading 2026-08-27: against LOWER counts r3's edge is per-trade quality (SR P <= 0.02 on both anchors; raw money unproven, hybrid vs r1 P = 0.48) carried by better timing on shared sessions, not by the marginal trades losing; against HIGHER counts it is money (P = 0.01-0.03) - the forfeited trades win ~55-59%. |
| `research_1m_sizing.py` | The quantized-sizing reading (2026-09-01): the published blotter and the matrix's `variant 4` replayed in INTEGER contracts against the idealized fractional replay. Policy since the same evening: `n = floor(equity x risk% / (rpu x point_value))`, and at n=0 the order is REFUSED at placement -- never forced -- and released as capital grows; default account $250,000 (the deployment scenario since 2026-09-02, shared with the contracts pages -- and the fractional variant pages render at $250k too, self-checking against run_1m's published figures at the $100k engine base; `--account 2000000` reproduces the everything-fits reading). Every taken side pays commission + exchange + NFA from `execution_costs.py` (sourced; taxes excluded). Specs from data_center's validated `metadata/contract_specs.json`; no engine passes, seconds. The ETF capital lock is account-size INDEPENDENT (`locked/equity = risk% x price/rpu`) and 5 of 12 ETF trades lock over 100% of the account -- the quantitative case in the futures-only-portfolio decision. Writes `output/quickfix1m1dc_sizing.txt`. |
| `build_1m_capital_report.py` | The capital ladder (2026-09-01): the published baseline's LIVE-UNIVERSE trades (the 22 futures we trade -- ETF and non-updated markets dropped before the replay, exact since the engine runs markets independently) deployed in integer contracts at $100k / $250k / $500k / $1M / $2M, one variant-format section per capital (equity/drawdown/open-positions panes, the refused orders, the full blotter with contracts, ACTUAL risk % and per-trade round-turn costs), opened by the 22-market contract arsenal (fronts + verified micros with whole-dollar min_account_1pct_stop) and the missed-trade trend (27 -> 16 -> 8 -> 3 -> 1; even $2M misses GC 2026-02-02 by $28 because the gate reads LIVE equity). TWO COMBINED pages since 2026-09-02 night: `output/quickfix1m1dc_capitals_variant_01.html` (baseline, published blotter) and `_04.html` (hybrid stop, matrix JSON), each section showing BOTH sizings -- the combined full+micro stack (accent curve + all headline figures, Stack/Drift/split-cost blotter columns) against full-contracts-only (thin gray reference curve, comparison KPI tile, "Missed, full-only" and "Final, full-only" trend columns). Micro routing: all eight sourced markets, drift priced per trade signed against the side (never a gate), no bar = no micro leg, top-ups capped at half the minute's volume, micro stops rounded away on the exchange's absolute grid on the parent's level-to-stop anchor; "The micro trade-offs" (the disadvantages) up front. Sections with no misses state so explicitly. First reading: $250k+ misses NOTHING on either variant; $100k misses only the no-micro markets (LE, PA, PL, 6J) where full-only missed 27-28. Refresh step `capitals1m` builds all four (micro pages skipped with a note when the gates JSON is absent). |
| `research_1m_liquidity.py` | Entry-minute liquidity, measured (2026-09-02): each live trade's exact entry minute's on-book volume from the contract's own bars vs the deployment sizes at $250k/$2M; participation = our contracts / contracts printed. First reading: thin markets (DX, PA, PL, ZW, HG, LE) breach the minute's print at size -- the fixed-slippage fill model is optimistic there; real capacity needs the order book (IB later). Writes `output/quickfix1m1dc_liquidity.txt`. |
| `research_1m_micro.py` | The micro drift-and-liquidity study (2026-09-02; drift PRICED not gated since the same evening): per sourced candidate, the drift profile (micro print vs parent print at OUR entry minutes, in parent ticks) plus a PER-ENTRY record of the signed basis and the micro's minute volume, from the $22.11 micro bars in data_center `data/_micros/`. All eight sourced markets route; the ladders book each trade's own drift into the curve (cost or credit), refuse a micro leg where no bar printed, and cap top-ups at half the minute's volume. Writes `output/quickfix1m1dc_micro_gates.{json,txt}`. |
| `execution_costs.py` | The execution-cost model (2026-09-01): per contract PER SIDE, IBKR fixed-rate commission + exchange fee + NFA, one row per market and per verified micro root, each with sources (IBKR's own worked examples; TradeStation and TradeProFutures passthrough lists, retrieved 2026-09-01), a confidence flag, and the higher figure wherever sources disagree. Unsourced rows carry None and refuse to price. Taxes excluded by decision; slippage stays in R (the fill), this is the bill. Standing instruction: replace rows with real IB statement lines when the account exists. |
| `research_1m_feasibility.py` | The $100k feasibility reading of the live-universe candidate (Lode's 22, 2026-09-01: 19 GLBX + FGBL kept despite EUR + SB, DX; JGB out, ETFs out of scope). Real per-contract risk from the v2/v5 blotters (wider anchor decides), the measured micro catalog as gap-filler, a labelled range estimate for untraded markets. First reading at $100k/1%: 13 full size, 6 more via micros, 3 gaps (PA, PL and -- the surprise -- SI, whose 1/5 micro is not small enough). Writes `output/quickfix1m1dc_feasibility.txt`; `--account`/`--risk`. |
| `build_1m_stop_profile.py` | ONE-TIME report (2026-09-03, Lode: "I thought we did a study around stoplosses and why we use it"): `output/quickfix1m1dc_stop_profile.html`, the engine's no-stop run beside the five dated measurements that made the stop's case (the GC pilot sweep, the trade review, the stop-anchor grid, the path analysis, the exit-timing pre-registration), figures recomputed from the JSONs on disk through the contracts pages' own replay, the five points as dated prose. Not in the refresh chain; rebuild by hand. Audit s.21 carries the cross-references. |
...
venv\Scripts\python.exe build_1m_report.py --variant "variant 4"   # any matrix cell, by number
venv\Scripts\python.exe build_1m_report.py --contracts       # both integer-contract pages ($250k, full + micro stack)
