...
| `research_1m_micro.py` | The micro gates (2026-09-02): fidelity (micro print vs parent print at OUR entry minutes, in parent ticks) and liquidity (the micro's own entry-minute volume vs the largest top-up stack) per sourced candidate, from the $22.11 micro bars in data_center `data/_micros/`. First reading: only MES and MNQ pass; the metal/grain micros drift 2-4 ticks (partly stale prints -- strict verdict stands until IB book data). Writes `output/quickfix1m1dc_micro_gates.{json,txt}`; the micro ladders consume the JSON. |
