...
| `build_1m_capital_report.py` | The capital ladder (2026-09-01): the published baseline deployed in integer contracts at $100k / $250k / $500k / $1M / $2M, one variant-format section per capital (equity/drawdown/open-positions panes, the refused orders, the full blotter with contracts, ACTUAL risk % and per-trade round-turn costs), opened by the contract arsenal (fronts + verified micros with whole-dollar min_account_1pct_stop) and the missed-trade trend (25 -> 15 -> 8 -> 2 -> 1; even $2M misses GC 2026-02-02 by $135 because the gate reads LIVE equity). Output `output/quickfix1m1dc_capitals.html`; refresh step `capitals1m`. |
